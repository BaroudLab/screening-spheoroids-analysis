...
def read_well(path,im_name,thresh=None,norm=None):
    """
    Parameters
    ----------
    ...
    if not thresh: #value not passed --> findthresh is calling, reading only PI
        ...
        PI = cv2.imread('\\'.join((path,'TRITC',im_name.replace('FITC','TRITC'))),-1)
        h,w = np.shape(PI)
        center = (w//2,h//2)      
        Y, X = np.ogrid[:h, :w]
    ...
    #reading pictures
    GFP = cv2.imread('\\'.join((path,'FITC',im_name)),-1)
    ...
    Res = pd.DataFrame(columns=['radius','gfp','pi','viability'])
    ...
    for sphero in spheroids:
        ...
        if 5<sphero.equivalent_diameter*.65/2<300: #avoiding too-little or full-well spheros
            ...
            Res = Res.append(pd.DataFrame([[Rad,GFP_lvl,PI_lvl,viab]],columns=Res.columns))
    return Res
